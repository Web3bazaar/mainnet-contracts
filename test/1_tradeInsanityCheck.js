...
contract(
  "Web3BazaarEscrow Contract - Check requirements ",
  async (accounts) => {
    ...
    it("Should set new max amount of assets per trade", async () => {
      ...
      const numMaxPerTrades =
        await BazaarEscrowInstance.numAssetsPerTrade.call();
    ...
    it("Starts trade between Wallet one and Wallet two successfully", async () => {
      const BazaarEscrowInstance = await Web3BazaarBatch.deployed();
      let bazCoinAddress = (await Bazcoin.deployed()).address;
      let erc721Address = (await TestCollection.deployed()).address;
    ...
    it("Execute trade should fail because wallet two doesn't allow web3bazaar contract to spend ERC721 tokens", async () => {
      ...
      try {
        ...
        const isReverting =
          error.message.search("WEB3BAZAAR_ERROR: CALLER INST THE EXECUTER") >=
          0;
    ...
    it("Should return an error cancelling the trade because the caller of this function is not the creator nor the executor", async () => {
      const tradeId = 1;
      const BazaarEscrowInstance = await Web3BazaarBatch.deployed();
      ...
      try {
        ...
        //console.log("error executing the trade ", error);
        const isReverting =
          error.message.search(
